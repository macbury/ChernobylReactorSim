import {ControlRod} from "./gameObjects/ControlRod.ts";
import {Neutron} from "./gameObjects/Neutron.ts";
import {WATER_SIZE} from "./gameObjects/Water.ts";

export class ControlRodsController {
  ...
  public setup(neutronsGroup : Phaser.Physics.Arcade.Group) {
    this.group = this.scene.physics.add.group({
      classType: ControlRod
    })
    
    const sX = WATER_SIZE / 2
    const sectorWidth = this.sectorSize * WATER_SIZE;

    for (let x = 0; x < this.count; x++) {
      const controlRod = new ControlRod(this.scene);
  ...
  private onNeutronHitControlRod(neutron : Neutron, controlRod : ControlRod) {
    controlRod.hit();
    neutron.destroy();
  }
}